...
- **Stage 2 — ARQ-prompted LLM extraction.** ARQ = **Attentive Reasoning Queries** (Karov, Zohar, Marcovitz, 2025 — [arXiv 2503.03669](https://arxiv.org/abs/2503.03669)): a structured-prompting technique where the model answers an ordered sequence of typed reasoning queries (slots) before emitting the final answer. Each slot focuses attention on one aspect of the input (visual audit, field grounding, ID/code verification) so reasoning becomes inspectable and reproducible instead of opaque.
