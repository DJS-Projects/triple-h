...
**Question:** Why migrate from single-pass extraction to ARQ (Anchored Retrieval+Queries) two-stage, when single-pass is "working"?
...
**My best inference from code/commits:** Single-pass LLM extraction is non-deterministic and gives no field provenance — a reviewer sees "value: X" with no way to verify which line in the source produced X. ARQ adds a deterministic Stage 1: Tier-1 walks the document looking for known field labels and grabs the nearest value; Tier-2 looks at table headers and aligns columns to fields. These anchors are then inlined into the Stage 2 LLM prompt so the model confirms verbatim instead of free-hallucinating, and emits per-field provenance (which anchor produced this field, with bbox). Per-doc-type ARQ schemas add reasoning slots for auditability. Gating behind `use_arq_pipeline` enables A/B comparison of cost, latency, and accuracy versus legacy single-pass. The flag's default-off posture lets the safer path remain live while ARQ is being validated.
